...
dec6 = pd.read_csv("datasets/coinmarketcap_06122017.csv") # Make sure to provide the correct file path

# Selecting the 'id' and the 'market_cap_usd' columns
market_cap_raw = dec6[['id', 'market_cap_usd']] # Use square brackets for column selection

# Counting the number of values
market_cap_count = market_cap_raw.count()
print(market_cap_count)
...
# Assuming market_cap_usd contains the market capitalizations
...
cap10 = market_cap_raw.sort_values(by='market_cap_usd', ascending=False).head(10)
cap10.set_index('id', inplace=True)

# Calculating market_cap_perc
total_market_cap = dec6['market_cap_usd'].sum()
cap10['market_cap_perc'] = (cap10['market_cap_usd'] / total_market_cap) * 100
...
ax = cap10['market_cap_usd'].plot(kind = 'line',title = TOP_CAP_TITLE)
...
largecaps = cap.query('market_cap_usd > 10000000000')  # This condition filters for market cap > 10 billion USD
...
# ... YOUR CODE # Create a bar plot
import matplotlib.pyplot as plt

plt.figure(figsize=(10, 6))
plt.bar(LABELS, values, color=['blue', 'green', 'red'])
plt.title('Distribution of Market Caps')
plt.xlabel('Market Cap Categories')
